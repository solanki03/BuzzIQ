...
app.get("/health", async (req, res) => {
  try {
    const mainDbCollections = await mongoose.connection.db
      .listCollections()
      .toArray();
...
app.post("/v1/results", async (req, res) => {
  try {
    ...
    if (
      result.correctAnswers + result.wrongAnswers + result.notAttempted !==
...
app.get("/v1/results/:userId/:topicname", async (req, res) => {
  try {
    ...
    const collections = await resultsDB.db
      .listCollections({ name: /^results_/ })
      .toArray();
  ...
  try {
    ...
    for (const coll of collections) {
      if (coll.name.startsWith("results_")) {
        const Model = resultsDB.model(coll.name, resultSchema);
        const userResults = await Model.find(
          { userId },
          {
            createdAt: 1,
            topic: 1,
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch chart data" });
  }
});
